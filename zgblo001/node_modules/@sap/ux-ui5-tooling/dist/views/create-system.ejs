...
<html>
    ...
    <body>
        <% if (systemCreationError.length) { %>
        <div class="error-text">
            <li>
                <% systemCreationError.forEach((e) => {%>
                    <ul><%= e %></ul>
                <% }); %>
            </li>
        </div>
        ...
        <div class="system-url"><%= systemUrl %></div>
        <div><%= client? 'Client: ' + client : '' %></div>
        <form action="/create-system" method="post">
            <div>
                <label for="systemName" class="req">System Name</label>
                <input type="text" id="systemName" name="systemName" class="system-name" autofocus required
                <% if (systemName) { %>
                    value="<%= systemName %>"
                <% } %>
                />
            ...
            <div>
                <label for="serviceKeysContents" class="req">Service keys contents</label>
                <textarea
                    id="serviceKeysContents"
                    name="serviceKeysContents"
                    placeholder="Paste your servicekey contents here"
                    class="uploadArea"
                    required
                    rows="15"><%= serviceKeysContents %></textarea>
            </div>
            <div>
                <label for="saveSystem">Save System</label>
                <input type="checkbox" id="saveSystem" name="saveSystem"></input>
            </div>
            <div>
                <input type="submit" value="Create System" class="button" />
            </div>
        </form>
