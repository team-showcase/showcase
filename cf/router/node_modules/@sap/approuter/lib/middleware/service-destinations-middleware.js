...
module.exports = function cacheServiceDestination(req, res, next) {
  if (!process.env.SAAS_APPROUTER || process.env.SAAS_APPROUTER_EXCLUDED_PATH === req.url){
    return next();
  }
  getTenant(req);
...
function getTenant(req){
  ...
  if (!process.env.TENANT_HOST_PATTERN){
    throw new Error('Tenant host pattern is not defined');
  }
  ...
  if (req.headers['x-approuter-authorization']){
    if (req.headers['x-subscriber-tenant']){
      ...
      let tokenDecoded = jwtDecode(req.headers['x-approuter-authorization']);
