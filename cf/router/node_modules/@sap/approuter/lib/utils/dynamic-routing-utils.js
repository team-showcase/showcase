  ...
  getVcapServicesData (req, appKey, function (err, vcapServicesData) {
    ...
    request.get(vcapServicesData.requestOptions, function onResponse(err, res, body) {
      if (err || !res || !res.statusCode) {
        ...
        var message = appKey.appPrefix ? 'Unauthorized. Please check with the business service \'' + appKey.appPrefix + '\' provider if the requested UI is defined as public' :
        ...
        if (vcapServicesData.requestOptions && vcapServicesData.requestOptions.headers['x-app-host-id']) {
          var appHostID = vcapServicesData.requestOptions.headers['x-app-host-id'];
...
function storeLogoutDataOnRequest (req, xsapp) {
  var routerConfigServices = req.app && req.app._store && req.app._store.mainRouterConfig && req.app._store.mainRouterConfig.appConfig
  && req.app._store.mainRouterConfig.appConfig.services ? req.app._store.mainRouterConfig.appConfig.services : null;

  if (!xsapp.destinations && !xsapp.services && !routerConfigServices) {
    return;
  }
  ...
  var approuterHost = req.headers.host ? req.headers.host : null;

  if (xsapp.destinations) {
    req.toBeCachedOnSession.user.logout.destinations = xsapp.destinations;
  }
  if (xsapp.services || routerConfigServices) {
    ...
    if (xsapp.services) {
      servicesNames = Object.keys(xsapp.services);
      for (i = 0; i < servicesNames.length; i++) {
        serviceName = servicesNames[i];
        req.toBeCachedOnSession.user.logout.services[serviceName] =
          Object.assign({approuterHost: approuterHost}, xsapp.services[serviceName]);
      }
    }
  }
}
...
function getVcapServicesData (req, appKey, cb) {
  ...
  if (!req.app || !req.app.services || !req.app.services[html5RepoService] || !req.app.services[html5RepoService].token || !req.app.services[html5RepoService].token.accessToken) {
    return cb ('Missing html5AppsRepo token');
  }

  html5RepoCredentials = businessServiceUtils.getCredentials('html5-apps-repo-rt', false, req);
...
function getApplicationKey (req) {
  ...
  if (!parts || parts.length < 2) {
    ...
    appKey.appDestinationId = parts[0];
...
function isDynamicRouting () {
  var html5RepoCredentials = businessServiceUtils.getCredentials('html5-apps-repo-rt', false);
...
function getHTML5RepoToken(req,html5RepoCredentials,cb){
  if (req.app.services[html5RepoService].token.tokenRefreshTimestamp - Date.now() <= 0){
    tokenUtils.loadClientCredentialsToken(req.app, html5RepoCredentials, html5RepoService, function(err){
      return cb(err, req.app.services[html5RepoService].token);
    });
  } else {
